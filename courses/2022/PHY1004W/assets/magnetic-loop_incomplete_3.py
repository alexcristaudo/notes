...
def makeloop(rloop, loopradius):
    ...
    for theta in arange(0.0, 2.0 * pi, dtheta):
        loop.append(rloop + vector(loopradius*sin(theta),
                    0.0, loopradius*cos(theta)))
...
def Field(r, loop):
    ...
    for i in range(len(loop_positions)-1):
        dR = (loop_positions[i]['pos']+loop_positions[i+1]
              ['pos'])/2 - r     # distance r to loop segment
        dI = loop_positions[i]['pos'] - \
            loop_positions[i-1]['pos']           # delta I
...
trail = curve(pos=r, radius=0.005)
...
while t < 30:
    ...
    # F = q*cross(v, B)
    # v += (F*dt)/m
    # r += v*dt -> remember we need v not just (F*dt*dt)/m. Or can use momentum then p/m * dt
    ...
    # MOST EFFICIENT
    v += sintheta * cross(v, Bhat) + (1 - costheta) * \
        cross(cross(v, Bhat), Bhat)
    r += v*dt

    trail.append(pos=r)
    KE.plot(pos=(t, 0.5*m*mag(v)**2))
