...
visTRAIL = False  # each ball leaves a trail
visREL = False  # display cm and vcm (True or False)
visTOT = False  # instantaneous v of each ball
visCM = False  # display arrows for vrel (1=True or 0=False)
...
interval = 10
retain = 40
b1 = sphere(pos=vector(-xmax, 0.8, 0), radius=0.5, color=color.blue,
            m=1, make_trail=visTRAIL, interval=interval, retain=retain)
rod1 = cylinder(pos=b1.pos, axis=vector(0, L0, 0),
                radius=0.1, color=vector(0.7, 0.5, 0))
b2 = sphere(pos=rod1.pos + rod1.axis, radius=0.5, color=color.blue,
            m=1, make_trail=visTRAIL, interval=interval, retain=retain)

b3 = sphere(pos=vector(-xmax, -2-L0, 0), radius=0.5, color=color.red,
            m=1, make_trail=visTRAIL, interval=interval, retain=retain)
rod2 = cylinder(pos=b3.pos, axis=vector(0, L0, 0),
                radius=0.1, color=vector(0.7, 0.5, 0))
b4 = sphere(pos=rod2.pos + rod2.axis, radius=0.5, color=color.red,
            m=1, make_trail=visTRAIL, interval=interval, retain=retain)
...
v1tot = arrow(pos=b1.pos, axis=vscale*vcm, color=b1.color,
              shaftwidth=0.2, fixedwidth=1, visible=visTOT)
v2tot = arrow(pos=b2.pos, axis=vscale*vcm, color=b2.color,
              shaftwidth=0.2, fixedwidth=1, visible=visTOT)
v3rel = arrow(pos=b3.pos, axis=vector(0, 0, 0), color=color.green,
              shaftwidth=0.2, fixedwidth=1, visible=visREL)
v4rel = arrow(pos=b4.pos, axis=vector(0, 0, 0), color=color.green,
              shaftwidth=0.2, fixedwidth=1, visible=visREL)
vcma1 = arrow(axis=vector(0, 0, 0), color=color.cyan,
              shaftwidth=0.2, fixedwidth=1, visible=visCM)
vcma2 = arrow(axis=vector(0, 0, 0), color=color.cyan,
              shaftwidth=0.2, fixedwidth=1, visible=visCM)
v3tot = arrow(pos=b3.pos, axis=vector(0, 0, 0), color=b3.color,
              shaftwidth=0.2, fixedwidth=1, visible=visTOT)
v4tot = arrow(pos=b4.pos, axis=vector(0, 0, 0), color=b4.color,
              shaftwidth=0.2, fixedwidth=1, visible=visTOT)

s = 'Which object has the greater total momentum? Greater kinetic energy? Greater translational kinetic energy?'
question = label(pos=vector(0, yquestion, 0), text=s,
                 height=hfont, color=color.black, box=0, visible=False)
...
while True:
    ...
    # wraparound from right edge of screen to left edge
