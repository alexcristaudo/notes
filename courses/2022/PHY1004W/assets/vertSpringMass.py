...
Mspring = 0.0081                    # mass of spring in kilograms (ps)
Mbob = 0.1                          # mass of bob in kilograms (p0)
Meffective = Mbob + Mspring/3       # effective mass (pe)

k = 7.386  # 3.693                     # spring constant in N/m
A = 0.01                            # amplitude of motion in metres

support = box(pos=top+vector(0, 0.001, 0),
              size=vector(0.040, 0.002, 0.040), color=color.green)
spring = helix(pos=top, axis=vector(0, -Lspring, 0), coils=20, radius=Rspring,
               thickness=Dspring, length=Lspring, color=vector(1, 0.7, 0.2))
bob = cylinder(pos=vector(0, 0, 0), vel=vector(0, 0, 0), mass=Mbob,
               axis=vector(0, -0.02, 0), radius=0.008, color=color.red)
