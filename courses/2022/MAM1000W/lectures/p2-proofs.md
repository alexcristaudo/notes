...
status: needs-review
source: latex
assets: [MAM1000W_P2.tex]
---
> [!warning]
> Compiled from **MAM1000W_P2.tex** by the built-in LaTeX renderer — Unhandled commands left as-is: \Delta
> The original .tex is attached above.

# P2 Proofs

## $\int k f(x) d x=k \int f(x) d x$ where $k$ is any number.
...
## $\int_{a}^{b} c d x=c(b-a), c$ is any number.
...
## If $f$ is an odd function, then $\int \limits _{-a}^a f(x)dx = 0$
...
## If $f$ is an even function, then $\int \limits _{-a}^a f(x)dx = 2\int \limits _{0}^a f(x)dx$
