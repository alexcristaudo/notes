...
Now let $h \in \bigcup \limits _{g \in G} gTg^{-1}$, then $h \in gTg^{-1}$ for some $g\in G$ and so $h = gtg^{-1}$ for some $t \in T$. Since $T \subseteq G$, we get $gtg^{-1}=h \in G$  and thus  $\bigcup \limits _{g \in G} gTg^{-1} \subseteq G$\\ So we get that $G = \bigcup \limits _{g \in G} gTg^{-1}$ as required
