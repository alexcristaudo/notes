...
\begin{figure}[hbt!]
        \centering
        \includegraphics[width=0.52\linewidth]{regression_assets/histogram.png}
        \caption{Density of Particulate Matter}
        \label{fig:b1}
    \end{figure} 
\subsubsection*{Pairwise Plots}
Pairwise scatterplots were generated for all continuous variables in the dataset. This process involved three steps:
\begin{enumerate}
    \item Identifying and selecting all numeric columns in R.
    \item Creating a new data frame containing only these continuous variables.
    \item Utilizing the 'ggpairs' function to generate the pairwise plots.
\end{enumerate}
\par\noindent\rule{\textwidth}{0.4pt}
\vspace{-25pt} % Adjust this value to reduce space
\begin{minted}{r}
...
\begin{figure}[hbt!]
        \centering
        \includegraphics[width=0.9\linewidth]{regression_assets/pairwise.png}
        \caption{Pairwise Scatter Plots}
        \label{fig:b1}
    \end{figure} 
\newpage
\subsubsection*{Categorical Variable Plots}
...
\begin{figure}[hbt!]
        \centering
        \includegraphics[width=1\linewidth]{regression_assets/categorical_variable.png}
        \caption{Categorical Boxplots}
        \label{fig:b1}
    \end{figure} 
\subsubsection*{Categorical Relationships}
...
\begin{figure}[hbt!]
    ...
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{regression_assets/holiday_industrial.png} % Path to the second image
        
        \caption{Holiday and Industrial Activity interaction}
        \label{fig:image1}
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{regression_assets/holiday_week.png} % Path to the second image
        \caption{Holiday and Day of the Week interaction}
        \label{fig:image2}
    \end{minipage} \hfill
\end{figure}
\begin{figure}[hbt!]
        ...
        \centering
        \includegraphics[width=0.5\textwidth]{regression_assets/industrial_week.png} % Path to the first image
        \caption{Industrial Activity and the Day of the Week interaction}
        \label{fig:image3}
...
% \begin{figure}[hbt!]
%         \centering
%         \includegraphics[width=0.75\linewidth]{regression_assets/holiday_week.png}
%         \caption{Interaction between Holiday and Day of the Week}
%         \label{fig:b1}
%     \end{figure} 
...
\begin{itemize}
    \item Based on Figure 1, the distribution of particulate matter appears to follow a normal distribution. 
    \item Figure 2 suggests that both humidity and traffic density have positive effects on particulate matter levels, with humidity showing the strongest influence, demonstrated by a moderate correlation coefficient of 0.547. On the other hand, urban greenery seems to have a negative impact on the particulate matter concentration. The strength of this is moderate with a correlation coefficient of -0.355. Additionally, the correlation coefficient between temperature and humidity is 0.224, suggesting a potentially weak collinearity between these two factors. 
    \item Figure 3 suggests a potentially positive relationship between industrial activity and particulate matter levels. Other variables examined in the study appear to have no significant effect.
    \item The interaction plots provide additional information about the relationships between variables. They show no significant interaction between the industrial activity and holiday status, nor between holiday status and days of the week. The relationship between industrial activity and the day of the week appears more complex, but ultimately, no significant interaction is present.
\end{itemize}
...
\end{Verbatim}
After this, we find the estimated variance $s^2 = \frac{(Y - \hat{\beta}_0\mathbf{1} - \hat{\beta }_1 X)^T(Y - \hat{\beta}_0\mathbf{1} - \hat{\beta }_1 X)}{n-k}$ for $n=$ num\_obs ($= 150$), $k = 2$
\par\noindent\rule{\textwidth}{0.4pt}
\vspace{-25pt} % Adjust this value to reduce space
\begin{minted}{r}
...
C_mat <- solve(t(X_mat) %*% X_mat) # Solving for the C matrix
...
\end{Verbatim}
\subsubsection*{Simultaneous Hypothesis Tests}
...
confint(model, level = 0.95) |> signif(3) # Displaying the 95% confidence interval
\end{minted}
\vspace{-15pt} % Adjust this value to reduce space
\par\noindent\rule{\textwidth}{0.4pt}
\begin{verbatim}
...
\begin{enumerate}
    ...
    \item \textbf{Industrial Activity: Moderate.} \\
    ...
    \item \textbf{Temperature:} \\ The hypothesis test for temperature yielded a p-value of 0.7398, which is not statistically significant at the conventional 0.05 level. We cannot conclude that temperature has a significant effect on particulate matter levels in this model.
    \item \textbf{Humidity:} \\ The test for humidity's effect resulted in a highly significant p-value of 1.684e-12. This provides strong evidence that humidity has an effect on particulate matter levels. The nature and magnitude of this effect directly is not analysed.
    \item \textbf{Overall Industrial Activity:} \\
    ...
    \item \textbf{Interaction between Huidity and Temperature:} \\ The confidence interval for this interaction contains 0, suggesting that there is no significant interaction effect on particulate matter levels
    \item \textbf{The other parameters} are not significant as their confidence intervals of not contain 0. Specifically, 
\end{enumerate}
...
\newpage
\section{Part Two: Simulation}
\subsection{Scenario A: Violation of Normality}
\subsubsection*{Methodology}
We conduct 1000 independent trials. For each simulation:
\begin{enumerate}
    \item We generate $num\_obs = 150$ residual points from a uniform distribution with mean 0 and variance $\sigma^2 = 100$, deliberately introducing a violation of the normality assumption (the parameters of the uniform distribution is described in Simulation below). This introduces a violation of normality of the residuals.
    \item The response variable $Y$ is calculated as $Y = \beta_0 + \epsilon = 30 + \epsilon$
\end{enumerate}
...
\begin{enumerate}
    \item Generate the residuals and calculate $Y$
    \item Fit a linear model of $Y$ against the temperature
    \item Extract the p-value associated with the temperature coefficient. If the p-value is below 0.05, we increment a counter
    \item The proportion of rejected tests (p-value < 0.05) out of the total simulations is then calculated and reported
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Create $num\_obs$ error points from normal distributions, with varying variances. This creates the violation of homoscedasticity
...
\subsection{Scenario C: Violation of Independence}
We conduct 1000 independent trials. For each simulation:
\begin{itemize}
    \item Utilize the provided generate\_ar1\_errors function to create correlated error terms. This is done using arguments $n = num\_obs, \rho = 0.3, var\_epsilon = 100$. This introduces a violation of independence.
    \item Calculate $Y = \beta_0 + \epsilon$
\end{itemize}
...
The simulation procedure for each trial is as follows:
\begin{itemize}
    \item Generate correlated error terms using the generate\_ar1\_errors function.
    \item Construct the response variable $Y$ using these error terms.
    \item Fit a linear model using the lm function.
    \item Extract the p-value associated with the temperature coefficient. If the p-value is below 0.05, increment a counter
\end{itemize}
The proportion of rejected tests (p-value < 0.05) out of the total simulations is then calculated and reported. The R code implementing this simulation is as follows:
\par\noindent\rule{\textwidth}{0.4pt}
\vspace{-25pt} % Adjust this value to reduce space
...
\begin{itemize}
    \item Non-normal residuals: 4.3\% rejection rate
    \item Heteroscedasticity: 5.6\% rejection rate
    \item Correlated errors: 4.6\% rejection rate
\end{itemize}
