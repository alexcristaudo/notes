...
\usepackage{enumitem}
\begin{document}
\includepdf{assets/GraphsTitle.pdf}
% \usepackage{graphicx}
\graphicspath{ {./assets/} }
...
\begin{itemize}
    \item \textbf{GraphException}. This is used for raising an Exception that may occur whilst using a graph object, such as "Graph has negative edges" or that the text file used to create the graph is incorrectly formatted
    \item \textbf{Edge}. This represents an edge in the graph, storing the destination vertex of the edge and the cost of that edge. So the \textbf{\textit{Vertex}} class is used as the destination instance variable
    
    \item \textbf{Path}. This is used to create a Path to be used in the Priority Queue in Dijkstra's Algorithm and uses \textbf{\textit{Vertex}} to store a destination vertex and also stores the cost for that path. Each path object also uses \textbf{\textit{Path}} to compare 2 different path objects.
    
    \item \textbf{Vertex}. This class represents a Vertex stored in the graph, storing its name, a list of edges, the cost for the shortest path and the previous vertex on the shortest path. This thus uses \textbf{\textit{Edge}} to store a list of edges with that vertex as the start node and \textbf{\textit{Vertex}} to store the previous vertex in the shortest path from Dijkstra's algorithm
    
\end{itemize}

The following classes were created/modified and interact as follows:
\begin{itemize}
    \item \textbf{Counter}. This class is used to store \textit{eCount} for edge access operations, \textit{vCount} for vertex access operations and \textit{pCount} for Priority Queue operations (insertion and deletion) used in Dijkstra's algorithm. The number of vertices for a graph and the number of edges are also stored. This class is thus used to keep track of and increment these counters whilst running Dijkstra's Algorithm and then create a text file for the stored values.
    
    \item \textbf{Graph}. This is to represent a graph with vertices and edges. The class loads data from a text file and processes the data to add the relevant vertices and edges in the graph instance. This class also has an implementation of Dijkstra's Algorithm and uses a \textbf{\textit{Counter}} object to count the \textit{eCount}, \textit{vCount} and \textit{pCount} operations in the algorithm. Since this class contains all vertices (each storing its edges), this graph is also used to get a list of valid vertices that can be used as a start node. Any vertex can be used as a start node if they connect to at least one other vertex. Therefore this class uses \textbf{\textit{Vertex}} to store vertices in the graph, \textbf{\textit{Path}} in a Priority Queue, \textbf{\textit{Edge}} create edges and access edges in Dijkstra's Algorithm and GraphException to raise exceptions. 
    
    \item \textbf{GraphExperiment}. This is the class used to perform the experiment and analyse the performance of Dijkstra's Algorithm. The class generates a random data set of nodes and edges and this involves generating a set of vertices by generating names, generating a set of edges for those vertices and writing the edges into a text file in the form $(A, B, n)$ where $A$ is the start node, $B$ is the end node and $n$ is the cost. The class is then also used to run Dijkstra's Algorithm and save the final output into text files. This class uses \textbf{\textit{Vertex}} to create random vertices, and also uses \textit{\textbf{Graph}} to create a graph object from a text file. This object runs Dijkstra's Algorithm and is used to get the performance of the algorithm
...
\begin{itemize}
    \item The values for $V$ are chosen from $\{10,20,30,40,50\}$. For each of these values, the corresponding amount of vertices is generated with the name \textit{Node..} where .. is the number of that node. For example, for 10 vertices, 10 vertex objects are created with names \textit{Node0, Node1, ..., Node9}.
...
\begin{center}
    \underline{test1\_1.txt} \\
    \lstinputlisting{assets/test1\_1.txt} 
\end{center}
...
\begin{tabular}{ccc}
    \underline{xData.txt} & \underline{yActual.txt} & \underline{yExpect.txt} \\
    \lstinputlisting{assets/xData.txt} ~~~~~~~~~~~~~~~~~~~~~~~~~~&
    \lstinputlisting{assets/yActual.txt} ~~~~~~~~~~~~~~~~~~~&
    \lstinputlisting{assets/yExpect.txt} \\
     
\end{tabular} \\ ~\\ We can see for test case 1, the average of \textit{vCount + eCount + pCount} $= 46,42857142857143$, and $|E| \log |V| = 66,43856189774725$ \\ ~\\
...
\includegraphics[scale = 0.71]{graph2}
...
\section{Creativity}
\begin{itemize}
    \item When generating random edges, HashMaps and HashSets are used to increase the performance of the code with both data structures, on average, finding data in $O(1)$ time.
    \item When selecting a random start node, the node is selected from a list of nodes that have some unpaired vertex. This means, no matter what start node is chosen, an end node can always be chosen. This will also increase the performance of the code as you no longer need to generate a random number until a valid Vertex is hit
    \item The end node is selected from a list of unpaired nodes. This means the end node is always selected with the first random number generation. This, along with the point above, means the start node and end node are always found after one random number generator for each
    \item The efficiency of the data set generation allows for much larger outputs to be produced, such as in Graph 2 above
    \item For each test case, sub-test cases are created and stored in a text file
    \item Every node that links to another node in a test case is used as a start node in Dijkstra's algorithm. This creates the sub-case test files and then the average for each of these test cases is taken improving the reliability of the outputted data that is plotted on the graph
    \item The data needing to be plotted is stored in a text file, with full stops replaced with commas to easily copy into some plotter such as Excel. 
    \item Every test case and the sub-test case is stored in a respective folder. The test cases are all stored in a 'test' folder, and the sub-test cases are all stored in an 'out' folder.
    \item The Makefile includes the option to automatically delete all the text files after use, by calling 'make text'. The Makefile also compiles the javadocs and creates them in a 'doc' folder, by running 'make javadoc'
\end{itemize}
\section{Git}
\begin{BVerbatim}
