---
title: MAM2000W — Linear Transformations
type: reference
tags: [latex]
status: needs-review
source: latex
...
---
> [!warning]
> Compiled from **template-2-3.tex** by the built-in LaTeX renderer — Unhandled commands left as-is: \cos, \sin, \vdots, \mathbf, \includegraphics, \item
> The original .tex is attached above.

# MAM2000W — Linear Transformations
...
>
> - $T(\mathbf{u}+\mathbf{v}) = T(\mathbf{u}) + T(\mathbf{v})$ for every $\mathbf{u}, \mathbf{v} \in V$
> - $T( \lambda \mathbf{u}) = \lambda T(\mathbf{u})$ for every scalar $ \lambda $ and vector $\mathbf{u} \in V$

> [!example]
> **Example**
>
> Let $A = $\begin{pmatrix} 1  & 2 & 3
> 1 & -3 & 2 \end{pmatrix}$ $. For every $ \mathbf{x} = $\begin{pmatrix} x_1
> x_2
> x_3 \end{pmatrix}$ \in \mathbb{R}^3$, let
>
>      $T(\mathbf{x}) = A \mathbf{x} = $\begin{pmatrix} x_1+2x_2+3x+3
> x_1-3x_2+2x_3 \end{pmatrix}$ $
>
...
> **Theorem**
>
> If $A$ is an  $m \times n$ matrix, then the function $T:\mathbb{R}^n \to \mathbb{R}^m$ defined by $T(\mathbf{u})=A\mathbf{u}$ for every $\mathbf{u} \in \mathbb{R}^n$ is a linear transformation

> [!warning]
> **Note**
>
> You probably already came across the idea that one could think of a matrix
...
>
> The matrices
> $\text{Rot}_\theta = $\begin{pmatrix} \cos \theta & -\sin \theta
> \sin \theta & \cos \theta \end{pmatrix}$ $, $\text{Proj}_x = $\begin{pmatrix} 1  & 0
> 0 & 0 \end{pmatrix}$ $, $\text{Ref}_x = $\begin{pmatrix} 1  & 0
> 0  & -1 \end{pmatrix}$ $
...
> **Note**
>
> We have that $f(x)=mx$ is linear, but  $f(x)=mx+c$ if not. We should actually call  $f(x)=mx+c$ affine functions, not linear functions

> [!warning]
> **Note**
>
> To show $T:V\to W$ is not linear, it is sufficient to find either:
>
> - 2 vectors $\mathbf{u},\mathbf{v} \in V$ such that $T(\mathbf{u}+\mathbf{v}) \neq T(\mathbf{u}) + T(\mathbf{v})$
> - A vector $\mathbf{u} \in V$ and scalar $ \lambda $ such that $T( \lambda \mathbf{u}) \neq \lambda T(\mathbf{u})$
...
>
> 1. $T( \lambda_1 \mathbf{u}_1 + \lambda _2 \mathbf{u}_2 + \cdots + \lambda_{n}\mathbf{u}_{n}) = \lambda _1 T(\mathbf{u}_1) + \lambda _2 T(\mathbf{u}_2) + \ldots+ \lambda _n T(\mathbf{u}_n)$
...
>
> ~
> 1. This uses a fairly simple induction
> 2. Since $\mathbf{0}+\mathbf{0}=\mathbf{0}$, we get that $T(\mathbf{0} + \mathbf{0}) = T(\mathbf{0}) \implies T(\mathbf{0}) + T(\mathbf{0}) = T(\mathbf{0})$, then adding the negative of $T(\mathbf{0})$ to both sides yields $T(\mathbf{0}) = \mathbf{0}$             
> [!warning]
> **Note**
>
...
> 3. Remember that $\mathbf{u}-\mathbf{v} = \mathbf{u} + (\mathbf{-v}) = \mathbf{u} + (-1)\mathbf{v}$ and then split as normal

> [!warning]
> **Note**
>
> We can use the second property to show a function is not linear

> [!theorem]
> **Corollary**
>
> Let $V$ and  $W$ be vector spaces and  $F:V \to W$ a function. If  $F(\mathbf{0}_V) \ne \mathbf{0}_W$, then $F$ is not a linear transformation

> [!example]
> **Example**
>
> $F:\mathbb{R}^2 \to \mathbb{R}^2$ defined by $F\left( $\begin{pmatrix} x_1
> x_2 \end{pmatrix}$  \right) = $\begin{pmatrix} x_1+1
> x_2-2 \end{pmatrix}$  $ is not a linear transformation because $F\left( $\begin{pmatrix} 0
> 0 \end{pmatrix}$  \right) = $\begin{pmatrix} 1
> -2 \end{pmatrix}$ \ne \mathbf{0} $

> [!warning]
> **Note**
>
> In the above case, we assume the vector spaces is the usual addition and multiplication for that set. Also if $F(\mathbf{0}) = \mathbf{0}$, then $F$ is not necessarily a linear transformation.
...
> **Claim**
>
> $\mathrm{ran} (T) = CS(A)$ in the above example

> [!example]
> **Myproof**
>
> Let $\mathbf{c}_1, \mathbf{c}_2, \ldots, \mathbf{c}_n$ be the columns of $A$ and  $\mathbf{v} =  $\begin{pmatrix} \lambda _1
> \lambda _2
> \vdots
> \lambda _n \end{pmatrix}$ $. Now $A \mathbf{v} \in \ran (T)$ if and only if $A \mathbf{v} = \left( \mathbf{c}_1, \mathbf{c}_2, \ldots, \mathbf{c_n} \right) $\begin{pmatrix} \lambda _1
> \lambda _2
> \vdots
> \lambda _n \end{pmatrix}$ = \lambda _1 \mathbf{c}_1 + \lambda _2 \mathbf{c}_2 + \ldots + \lambda _n \mathbf{c}_n $
>     if and only if $A\mathbf{v}$ is a linear combination of the columns of $A$ if and only if  $A\mathbf{v} \in CS(A)$.
>     
> [!warning]
> **Note**
>
> This is still an $m \times 1$ matrix because each column vector is an $m \times 1$ matrix
...
> Then use  $ \mathrm{ran}(T) = \mathrm{span}\left( \{ T(1), T(x), T(x^2), T(x^3) \} \right) $ but $T(1)$ and  $T(x)$ are linearly dependent, and  $T(x^2)$ and  $T(x^3)$ are linearly dependent, so we can take  $x,x^3$ as basis for  $ \mathrm{ran}(T)$
> (see notes)
...
>
> \begin{itemize}
>      $P_n$ (polynomials of degree at most $n$) and $\mathbb{R}^{n+1}$ are isomorphic because $T:P_n \to \mathbb{R}^{n+1}$ defined by $T(a_0+a_1x+\ldots a_nx^n) = (a_0,a_1,\ldots,a_n)$ is a linear isomorphism
>      $M_{2 \times 2} (\mathbb{R}) $ and $\mathbb{R}^4$ are isomorphic. Define isomorphism $T: M_{2 \times 2}(\mathbb{R}) \to \mathbb{R}^4$ by $$\begin{pmatrix}
>         a  &  b \\ c  &  d
...
> **Note**
>
> This is called a linear transformation because it is a function from one vector space to another that respects the underlying (linear) structure of each vector space
...
> **Note**
>
> We say that $A$ is the matrix which represents  $T$ with respect to the two bases $B_V$ and  $B_W$. We denote $~_{B_W}[T]_{B_V}$. If $V=W$ and we use the same basis  $B$ for  $V$  both times, $A$ is called the matrix which represents  $T$ with respect to  $B$
