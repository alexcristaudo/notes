...
\begin{enumerate}[(i)]
\item $\mu$ is a positive measure if and only if for any increasing sequence $(E_n)_{n\in\mathbb{N}} \subset \mathcal{E}$, 
\[
\mu\Big(\bigcup_{n=0}^{\infty} E_n\Big) \;=\; \lim_{n\to\infty}\mu(E_n) \;=\; \sup_{n}\mu(E_n)\,. 
\]
\item If $\mu$ is a positive measure, then for any decreasing sequence $(E_n)_{n\in\mathbb{N}} \subset \mathcal{E}$ with $\mu(E_0)<\infty$, 
\[
\mu\Big(\bigcap_{n=0}^{\infty} E_n\Big) \;=\; \lim_{n\to\infty}\mu(E_n) \;=\; \inf_{n}\mu(E_n)\,. 
\]
\item If for every decreasing sequence $(E_n)_{n\in\mathbb{N}} \subset \mathcal{E}$ satisfying $\mu(\bigcap_{n} E_n)=0$ we have $\lim_{n\to\infty}\mu(E_n)=0$, then $\mu$ is a positive measure.
\end{enumerate}
\end{theorem*}
...
\begin{corollary*}[Corollary 2.15]
...
\begin{enumerate}
\item the functions $\max\{f,g\}$, $\min\{f,g\}$, $f+g$, and $f\,g$ are measurable;
\item the positive and negative parts $f^+ := \max\{f,0\}$ and $f^- := \max\{-f,0\}$ are measurable;
\item the absolute value $|f|$ is measurable.
\end{enumerate}
\end{theorem*}

\begin{theorem*}[Theorem 2.17]
...
\begin{enumerate}[(1)]
\item $\displaystyle \int_X (f+g)\,d\nu \;=\; \int_X f\,d\nu + \int_X g\,d\nu$.
\item $\displaystyle \int_X (t\,f)\,d\nu \;=\; t \int_X f\,d\nu$.
\item If $f \le g$, then $\displaystyle \int_X f\,d\nu \;\le\; \int_X g\,d\nu$.
\end{enumerate}
\end{theorem*}

\begin{theorem*}[Theorem 3.5 (Beppo–Levi Monotone Convergence)]
...
\begin{theorem*}[Theorem 3.8 (Fatou’s Lemma)]
...
\begin{theorem*}[Theorem 3.12]
Let $(X, \mathcal{E}, \mu)$ be a measure space. 
\begin{enumerate}
\item If $f: X \to \mathbb{R}$ is $\mu$-integrable and $g: X \to \mathbb{R}$ is $\mu$-summable (i.e. $|g|$ is integrable), then $f+g$ is $\mu$-integrable and $\displaystyle\int_X (f+g) = \int_X f + \int_X g$.
\item If $f$ is $\mu$-integrable and $\alpha \in \mathbb{R}$, then $\alpha f$ is $\mu$-integrable and $\displaystyle \int_X \alpha f = \alpha \int_X f$.
\item If $f$ and $g$ are $\mu$-integrable and $f \le g$ $\mu$-a.e., then $\displaystyle \int_X f \le \int_X g$.
\end{enumerate}
\end{theorem*}

\begin{theorem*}[Theorem 3.14 (Dominated Convergence Theorem)]
...
\begin{enumerate}
\item $f$ is $\mu$-integrable (resp. $\mu$-summable) if and only if $g$ is $\mu$-integrable (resp. $\mu$-summable$)$.
\item In the case that both are integrable, $\displaystyle \int_X f\,d\mu = \int_X g\,d\mu$.
\end{enumerate}
\end{theorem*}
...
\begin{itemize}
\item for each fixed $y\in Y$, the function $x \mapsto f(x,y)$ is measurable and integrable on $X$;
\item for each fixed $x\in X$, the function $y \mapsto f(x,y)$ is continuous on $Y$;
\item there exists an integrable function $h(x)$ on $X$ such that $|f(x,y)| \le h(x)$ for all $y \in Y$ and all $x \in X$;
\end{itemize}
then the function $F: Y \to \mathbb{R}$ defined by $F(y) := \int_X f(x,y)\,d\mu(x)$ is continuous on $Y$.
\end{theorem*}

\begin{theorem*}[Theorem 3.17 (Differentiation Under the Integral Sign)]
Let $(X, \mathcal{E}, \mu)$ be a measure space and $(Y,d)$ a metric space. Suppose $f: X \times Y \to \mathbb{R}$ satisfies:
\begin{itemize}
\item for each $t \in Y$, $x \mapsto f(x,t)$ is integrable on $X$, and for each $x\in X$, the map $t \mapsto f(x,t)$ is differentiable on $Y$;
\item the partial derivative $\partial f/\partial t(x,t)$ exists for all $x,t$ and there is an integrable function $H(x)$ such that $\big|\frac{\partial f}{\partial t}(x,t)\big| \le H(x)$ for all $t\in Y$;
\end{itemize}
...
\begin{theorem*}[Theorem 3.26 (Hölder’s Inequality)]
...
\begin{theorem*}[Theorem 3.27 (Minkowski’s Inequality)]
...
\section{Normed Spaces and Linear Mappings}

\subsection*{Finite-Dimensional Spaces and Compactness}
\begin{theorem*}[Theorem 3.4]
Let $(V,\|\cdot\|)$ be a normed space of \emph{finite dimension}. Then a subset $K \subset V$ is compact if and only if $K$ is closed and bounded.
\end{theorem*}

\begin{lemma*}[Lemma 3.5 (Riesz’s Lemma)]
...
\subsection*{Banach Spaces and Continuous Linear Maps}
...
\begin{enumerate}
\item $L$ is continuous (as a function between metric spaces).
\item $L$ is continuous at $0_V$.
\item There exists a constant $C>0$ such that $\|L(v)\|_W \le C\,\|v\|_V$ for all $v \in V$.
\item $L$ is a Lipschitz continuous function on $V$.
\end{enumerate}
\end{theorem*}
...
\subsection*{Hilbert Space Results}
\begin{corollary*}[Corollary 6.6]
...
\begin{theorem*}[Theorem 6.10 (Parallelogram Identity)]
...
\begin{enumerate}
\item $\{e_n\}_{n\in\mathbb{N}}$ is complete in $H$ (i.e. an orthonormal basis of $H$).
\item $\displaystyle \|u\|^2 = \sum_{n=0}^{\infty} |(u,e_n)|^2$ for every $u \in H$ (Bessel’s identity becomes an equality).
\item $\displaystyle (u,v) = \sum_{n=0}^{\infty} (u,e_n)\,\,(v,e_n)$ for all $u,v \in H$ (Parseval’s identity).
\end{enumerate}
\end{theorem*}

\begin{theorem*}[Theorem 6.16 (Gram–Schmidt Process)]
...
\begin{theorem*}[Theorem 6.17 (Riesz Representation)]
...
\begin{theorem*}[Theorem 6.19 (Adjoint Operator)]
Let $(V,(\cdot,\cdot)_V)$ and $(W,(\cdot,\cdot)_W)$ be two Hilbert spaces over the same field $K$. For each linear continuous operator $L \in L(V,W)$, there exists a unique linear continuous operator $L^*: W \to V$ (called the \emph{adjoint} of $L$) such that 
\[
...
\subsection*{Analytic Forms of Hahn–Banach}
\begin{theorem*}[Theorem 2.2 (Hahn–Banach, Analytic Form over $\mathbb{R}$)]
...
\begin{theorem*}[Theorem 2.4 (Hahn–Banach, Analytic Form over $\mathbb{C}$)]
...
\begin{theorem*}[Theorem 3.1 (Hahn–Banach for Seminorms)]
...
\subsection*{Hahn–Banach in Normed Spaces and Consequences}
\begin{theorem*}[Theorem 4.1 (Hahn–Banach in a Normed Space)]
...
\begin{theorem*}[Theorem 5.4 (Hahn–Banach, First Geometric Form)]
...
\begin{theorem*}[Theorem 5.7 (Hahn–Banach, Second Geometric Form)]
Let $(V,\|\cdot\|_V)$ be a normed space over $K=\mathbb{R}$ or $\mathbb{C}$. Let $E$ and $F$ be two nonempty disjoint convex subsets of $V$, with $E$ closed and $F$ compact. Then there exists a closed hyperplane $H = \{L = \alpha\}$ (for some $L\in V'$ and $\alpha \in \mathbb{R}$) that \emph{strictly} separates $E$ and $F$. That is, there exists $\varepsilon > 0$ and $L \in V'$ such that 
\[ 
...
\section{Baire Category and Banach’s Theorems}

\subsection*{Baire’s Theorem}
\begin{theorem*}[Theorem 1.1 (Baire Category Theorem)]
...
\begin{theorem*}[Theorem 3.1 (Banach–Steinhaus Theorem)]
...
\begin{theorem*}[Theorem 4.3 (Banach Open Mapping Theorem)]
...
\subsection*{Closed Graph and Hellinger–Toeplitz Theorems}
\begin{theorem*}[Theorem 5.1 (Banach Closed Graph Theorem)]
...
\begin{theorem*}[Theorem 5.2 (Hellinger–Toeplitz Theorem)]
Let $(H,(\cdot,\cdot))$ be a Hilbert space, and let $T: H \to H$ be a linear operator which is \emph{symmetric}, meaning $(T v, w) = (v, T w)$ for all $v,w \in H$. Then $T$ is continuous (i.e. bounded as an operator on $H$). 
\end{theorem*}

\section{Projection on Convex Sets and Variational Inequalities}

\subsection*{Projection onto a Convex Set}
\begin{theorem*}[Theorem 2.1 (Projection onto a Closed Convex Subset)]
...
\begin{enumerate}[(a)]
\item $P_K(H) = K$ (range is $K$);
\item $\|P_K(u_1) - P_K(u_2)\|_H \le \|u_1 - u_2\|_H$ for all $u_1,u_2 \in H$ ($P_K$ is $1$-Lipschitz, in fact nonexpansive).
\item $P_K$ is idempotent: $P_K(u) = u$ for all $u \in K$, and hence $P_K(P_K(u)) = P_K(u)$ for all $u \in H$.
\end{enumerate}
\end{theorem*}

\subsection*{Stampacchia’s Theorem (Variational Inequality)}
\begin{theorem*}[Theorem 2.3 (Stampacchia’s Existence Theorem)]
Let $(H,(\cdot,\cdot))$ be a real Hilbert space with dual $H'$ and induced norm $\|\cdot\|_H$. Let $K \subset H$ be a nonempty closed convex subset. Let $\ell \in H'$ be a continuous linear functional, and let $a(\cdot,\cdot): H \times H \to \mathbb{R}$ be a continuous bilinear form which is \emph{coercive} and \emph{symmetric} on $H$.  Then there exists a unique $u \in K$ such that 
\[ 
...
\begin{theorem*}[Theorem 2.6 (Lax–Milgram Theorem)]
