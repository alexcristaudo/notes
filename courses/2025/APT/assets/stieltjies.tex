...
\]
\end{theorem}
\noindent This generalizes the usual integration-by-parts.  For example, if $g$ is differentiable, then $df(x)=f'(x)\,dx$ and one recovers the standard formula $\int f\,dg = f\,g - \int g\,df$.  Integration by parts is often useful in probability, for instance in relating expectation and distribution functions.

\section{Stieltjes Integration in Statistics}
...
\section{Examples of Expected Values via Stieltjes Integrals}
...
\begin{example}[Continuous Distribution]
...
\section{Integration by Parts for Stieltjes Integrals}
