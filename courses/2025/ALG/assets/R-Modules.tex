...
\input{NotesModified/preamble}

\begin{document}

\setcounter{section}{1} % start at 1 on first section
...
such that
\begin{enumerate}
    \item $(M, +)$ is an abelian group (with identity, say, $0$)
    \item For all $r,r_1,r_2\in R$ and all $m,m_1,m_2\in M$, the operation $\cdot $ satisfies
    \begin{center}
        $(r_1+r_2)\cdot m=r_1 \cdot m+r_2\cdot m$ 
        \\ $r\cdot (m_1+m_2)=r\cdot m_1+r\cdot m_2$ \\
        $r_1 \cdot (r_2 \cdot m)=(r_1 r_2)\cdot m$ \\ $1_R \cdot m=m$
    \end{center}
    This is called a left $R$-module (scalar multiplying on the left). Similarly, a right $R$-module multiplies on the right
\end{enumerate}
\end{definition}
...
\begin{enumerate}
    \item If $R = F$ is a field, then an $F$-module is a vector space over $F$
    \item If $R=\mathbb{Z}$, then a $\mathbb{Z}$-module is an abelian group where $\cdot :\mathbb{Z} \times A \to A$ maps 
    \begin{center}
        $(n,a) \mapsto \begin{cases}
            \overbrace{a+a+...+a}^{n \text{ copies}} & n>0 \\ 0 & n=0 \\ -(\underbrace{a+a+...+a}_{n \text{ copies}}) & n < 0
        \end{cases}$
    \end{center}
...
\begin{definition}{Kernels and Images}{}\\
...
\begin{definition}{Category Theory Definition}{}
\begin{enumerate}
...
\begin{thm}{Universal Property of Quotient Modules}{}\\
    \[
\begin{tikzcd}[row sep=large, column sep=large]
...
\end{prf}
So we can define
\begin{definition}{Cokernels in R-Modules}{} \\
...
\begin{thm}{About projections and injections}{} \\
...
% \begin{definition}{Direct Sum}{} \\ Let $M_1,...,M_n$ be $R$-modules. The direct sum
% \begin{center}
%     $M_1 \oplus M_2\oplus \  ... \ \oplus M_n $
% \end{center}
% is the set $M_1\times \ ... \times \ M_n$ with operations 
% \begin{center}
%     $\{m_i\}_{i=1}^n + \{m'_i\}_{i=1}^n = \{m_i+m'_i\}_{i=1}^n$ \\ 
%     $r \{m_i\}_{i=1}^n = \{rm_i\}_{i=1}^n$ \ \ $(r\in R)$
% \end{center}
% We will write $\oplus _{i=1}^nA_i$ as a shorthand
% \end{definition}

\begin{definition}{Categorical definition}{}\\
...
\newpage
\begin{thm}{The universal property}{}\\
...
\begin{definition}{Jointly epic and monic}{}\\
...
\begin{definition}{Coproduct of A and B}{}\\
...
\begin{definition}{Directed Sum Diagram}{}\\
...
$\iota _i(x_i)(j) = \begin{cases}
    0 & i \ne j \\ x_i & i=j
\end{cases}$ \\ and our external direct sum = coproduct $\sum _{i \in I}A_i = \{f:I \to \coprod_{i \in I} A_i : f(i)\in A_i\ , \{i \in I:f(i) \ne 0\} \text{ is finite}\}$
